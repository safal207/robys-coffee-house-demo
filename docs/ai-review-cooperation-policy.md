...
- a non-Bot account;
- a signal whose latest bot-controlled `created_at`/`updated_at` timestamp predates the active-provider request;
...
- provider-limit failover requires an authenticated allowlisted active Bot signal after that provider's current-head request;
...
| `PROVIDER_LIMIT` | Both active current-head requests exist and at least one requested allowlisted active-provider Bot reported a current-round limit | Select the other active exact-head reviewer; remain fail-closed if it has not reviewed |
