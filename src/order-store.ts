...
/** One relevant, optional accompaniment; food or a set already fills this need. */
export function suggestOrderAddition(lines: readonly OrderLine[]) {
  const products = lines.map(line => resolveOrderProduct(line.id)).filter(Boolean);
  const drinkCategories = new Set(['hot-coffee', 'cold-coffee', 'brew-hot', 'herbal-tea']);
  if (!products.some(product => drinkCategories.has(product!.category.id))) return undefined;
  if (products.some(product => ['food', 'desserts', 'pairing-offers'].includes(product!.category.id))) return undefined;
  const item = SMART_CHOICE_CATALOG.items.find(item => item.id === 'desserts--macaron' && item.sourceStatus === 'confirmed' && item.availability === 'available');
  const sku = item && sourceToSku.get(item.sourceId);
  const product = sku ? resolveOrderProduct(sku) : undefined;
  return product && item && Math.round(product.item.price * 100) === item.priceMinor ? product : undefined;
}
...
/** Expand checked portions into editable menu lines only when every displayed price agrees. */
export function linesFromChoice(cart: CartState, partySize: PartySize): OrderLine[] {
  const calculation = calculateCart(cart, partySize);
  if (!calculation.canHandoff) throw new Error('Incomplete choice');
  const lines = calculation.lines.map(line => {
    const id = sourceToSku.get(line.itemId);
    const product = id ? resolveOrderProduct(id) : undefined;
    if (!id || !product || Math.round(product.item.price * 100) !== line.unitPriceMinor) throw new Error('Choice/menu identity mismatch');
    return { id, quantity: line.quantity };
  });
  const total = lines.reduce((sum, line) => sum + Math.round(resolveOrderProduct(line.id)!.item.price * 100) * line.quantity, 0);
  if (total !== calculation.totalMinor) throw new Error('Priced set cannot be split into menu items');
...
export function createOrderStore(
  ...
  const store = {
    ...
    addMany(lines: OrderLine[]) {
      const checked = validLines(lines);
      if (!checked.length) throw new Error('Empty addition');
      const next = new Map(snapshot.lines.map(line => [line.id, line.quantity]));
      for (const line of checked) next.set(line.id, (next.get(line.id) ?? 0) + line.quantity);
      // Validate the entire addition before changing a single line or emitting an update.
      replace(validLines([...next].map(([id, quantity]) => ({ id, quantity }))));
    },
    undoRemoval() {
      if (!undo) return;
