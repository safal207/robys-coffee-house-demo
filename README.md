...
The first active pattern is **business-truth drift**: a customer-facing value can exist in the canonical profile before its owner-confirmation state is explicit. The current ledger binds all eleven owner-confirmed profile values to SHA-256 digests in `production` publication mode, and the gate fails closed if a value changes or an owner-critical field loses confirmation.
