...
assert(Array.isArray(pairingOfferCatalog), "pairingOfferCatalog must be an array");

const publicPairingCategory = menuCategories.find((category) => category.id === "pairing-offers");
assert(publicPairingCategory, "Public pairing category is missing");
assert(
  publicPairingCategory.items.every((offer) => isPublicPairingEligible(offer.journeyId)),
  "Public pairing category contains an unavailable or unconfirmed offer"
);
assert(!isPublicPairingEligible("unknown-pairing"), "Unknown pairing policy must fail closed");

const sourceCategories = menuCategories.map((category) =>
...
const publicItemCount = menuCategories.reduce(
  (count, category) => count + (category.items ?? category.groups.flatMap((group) => group.items)).length,
  0
);

console.log(
  `✅ MENU-CONTENT-001 passed: ${baseline.categoryCount} categories, ${itemCount} source items, ` +
  `${publicItemCount} publicly eligible items, ${baseline.languages.length} languages, all verified source prices unchanged.`
);
