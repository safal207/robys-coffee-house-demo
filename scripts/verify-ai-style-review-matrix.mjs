...
record("claude", [
  ...
  ["AI evidence is tied to the active request-bound pool and current head", aiWorkflow.includes("verify-ai-review-contract.cjs") && aiContract.includes("currentHead") && aiContract.includes("commit_id") && aiContract.includes("QODO_COMMAND") && aiContract.includes("CODEX_COMMAND") && aiContract.includes("warmStandbyRoundReady") && aiContract.includes("DORMANT_PROVIDER_NAMES")]
]);
