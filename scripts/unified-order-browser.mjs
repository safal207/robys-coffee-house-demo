 ...
 if(baseline){
  ...
  for(const language of ['tr','en','ru']){
   ...
   try{
    await check(language+' recommendation adds to shared order and the bar accepts a real press',async()=>{
     ...
     await p.locator('#robys-order-dialog').waitFor({state:'visible'});
  ...
  await check('320px large text and desktop order layout do not overflow',async()=>{
   for(const width of [320,1440]){
    ...
    await p.evaluate(()=>{document.documentElement.style.fontSize='32px';});await p.locator('#robys-order-dialog').waitFor({state:'visible'});
