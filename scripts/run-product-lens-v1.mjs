...
const EXPECTED_BOUNDARY_CHECKS = Object.freeze({
  paidExtrasPreselected: false,
  onlinePaymentPresent: false,
  hiddenRecurringTermsDetected: false,
  falseUrgencyDetected: false,
  obstructedDeclineDetected: false,
  growthClaimAuthorized: false
});
const EXPECTED_AUTHORITY = "The lens and its labels are advisory. They do not authorize pricing, payment, launch, experiment, deployment, delivery, or merge.";
const EXPECTED_HANDOFF_KINDS = Object.freeze({
  "index.html": ["maps", "instagram"],
  "menu.html": ["maps", "instagram"],
  "social-offer.js": ["instagram"],
  "src/social-offer.ts": ["instagram"]
});
...
const discoverRotationJs = sources["discover-rotation-v3.js"] || "";
const discoverRotationTs = sources["src/discover-rotation.ts"] || "";
...
const externalLinks = Object.values(externalLinksBySource).flat();
const expectedHandoffCoverage = Object.entries(EXPECTED_HANDOFF_KINDS).every(([pathname, expectedKinds]) => {
  const foundKinds = new Set((externalLinksBySource[pathname] || []).map(externalKind).filter(Boolean));
  return expectedKinds.every((kind) => foundKinds.has(kind));
});
const handoffCoverageDetail = Object.entries(EXPECTED_HANDOFF_KINDS).map(([pathname, expectedKinds]) => {
  const foundKinds = [...new Set((externalLinksBySource[pathname] || []).map(externalKind).filter(Boolean))].sort();
  return `${pathname}=expected:${expectedKinds.join("+")},found:${foundKinds.join("+") || "none"}`;
}).join("; ");
...
const generatedDiscoverRotationJs = discoverRotationTs ? compileClassicScript(discoverRotationTs) : "";
const discoverRotationParity = Boolean(generatedDiscoverRotationJs) && generatedDiscoverRotationJs === discoverRotationJs;
const buildContractMatches = buildScript.includes('transpileClassicScript("src/social-offer.ts", "social-offer.js")') &&
  buildScript.includes('transpileClassicScript("src/discover-rotation.ts", "discover-rotation-v3.js")') &&
...
const checks = [
  ...
  check("BUILD-001", "Generated-script build contract remains pinned", buildContractMatches, ["scripts/build.mjs", "trusted TypeScript compiler"], "social-offer and active discover renderer must transpile as strict ES2020 classic scripts with comments preserved"),
  ...
  check("DISCOVER-001", "Entire active Taste Journey renderer matches typed source", discoverRotationParity, ["src/discover-rotation.ts", "discover-rotation-v3.js", "scripts/build.mjs", "discover.html"], `generatedSha256=${generatedDiscoverRotationJs ? sha256(generatedDiscoverRotationJs) : "missing"}; runtimeSha256=${discoverRotationJs ? sha256(discoverRotationJs) : "missing"}`),
  ...
  check("PATH-001", "Entry-to-visit path remains present", indexHtml.includes("menu.html#pairing-offers") && socialOfferJs.includes('link.href = "discover.html"') && indexExternalKinds.has("maps") && indexExternalKinds.has("instagram") && menuHtml.includes("data-instagram-booking") && menuHtml.includes('data-menu-action-copy="mapsLink"') && discoverHtml.includes('id="pairing-menu-link"') && discoverHtml.includes("discover-rotation-v3.js"), ["index.html", "menu.html", "menu-actions.js", "discover.html", "discover-rotation-v3.js", "social-offer.js"], "homepage → pairing/menu → active Taste Journey renderer → Maps or Instagram handoff"),
  check("OFFER-001", "Optional pairing prices remain explicit", menuData.includes('id: "pairing-offers"') && menuData.includes('pricingMode: "approved-offer"') && menuData.includes('pricingMode: "menu-total"') && /price:\s*290\b/.test(menuData) && /price:\s*370\b/.test(menuData) && pairingPosters.includes("pairing-poster-price") && discoverRotationTs.includes('localized("Fiyat: 290 ₺", "Price: 290 ₺", "Цена: 290 ₺")') && discoverRotationJs.includes('localized("Fiyat: 290 ₺", "Price: 290 ₺", "Цена: 290 ₺")'), ["menu-data.js", "pairing-posters.js", "src/discover-rotation.ts", "discover-rotation-v3.js"], "approved offer and menu-total pairings expose TRY prices in source and active renderer"),
  check("CHOICE-001", "No online payment or preselected paid extra", /form-action 'none'/.test(menuHtml) && !/<input[^>]+\bchecked\b/i.test(`${indexHtml}\n${menuHtml}\n${discoverHtml}`) && !/(stripe|paypal|checkout\.com|iyzico|payment[_-]?intent)/i.test(`${indexHtml}\n${menuHtml}\n${menuActionsJs}\n${discoverHtml}\n${discoverRotationJs}\n${discoverRotationTs}\n${socialOfferJs}\n${socialOfferTs}`), ["index.html", "menu.html", "menu-actions.js", "discover.html", "discover-rotation-v3.js", "src/discover-rotation.ts", "social-offer.js", "src/social-offer.ts"], "pairings are presentation and discovery, not an online payment commitment"),
  ...
  check("CLAIM-001", "Offer claims remain unproven rather than promoted to growth evidence", socialOfferParity && discoverRotationParity && socialOfferJs.includes("price: 340") && socialOfferTs.includes("price: 340") && socialOfferJs.includes('currency: "₺"') && socialOfferTs.includes('currency: "₺"') && analyticsJs.includes('event: "robys_action"') && contract.overallConclusion === "VALUE_UNPROVEN", ["social-offer.js", "src/social-offer.ts", "discover-rotation-v3.js", "src/discover-rotation.ts", "scripts/build.mjs", "analytics.js", "qa/product-lens.v1.json"], "explicit source/runtime offers exist, but realized-value outcome is not evidenced")
];
...
const exactHead = {
  ...
  trustedLensHead,
  contractLensHead,
  lensMatches: lensHeadMatches
};
const causalGraph = {
  nodes: contract.journey,
  edges: [
    { from: "entry", to: "discovery", state: "PROVEN", evidence: ["index.html", "discover.html"] },
    { from: "discovery", to: "menu", state: "PROVEN", evidence: ["discover-v2.js", "discover-rotation-v3.js", "src/discover-rotation.ts", "menu.html"] },
    { from: "menu", to: "pairing", state: "PROVEN", evidence: ["menu-data.js", "pairing-posters.js", "discover-rotation-v3.js", "src/discover-rotation.ts"] },
