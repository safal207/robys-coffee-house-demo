...
async function measureCold(browser, scene) {
  ...
  assert(
    events.some((event) => event.scene === scene && event.state === "brand-frame" && event.variant === "cold"),
    `${scene} cold BRAND_FRAME event missing`
  );
  assert(
    events.some((event) => event.scene === scene && event.state === "handoff" && event.variant === "cold"),
...
try {
  ...
  const crossSceneWarmMs = Date.now() - warmStartedAt;
  ...
  assert(
    continuityEvents.some((event) => event.scene === "night" && event.state === "brand-frame" && event.variant === "warm"),
    "Night did not inherit warm replay state after Day"
  );
