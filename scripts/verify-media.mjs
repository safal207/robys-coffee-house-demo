...
const activeSource=extractSingleHeroVideoSource(indexHtml,'index.html');
...
if(ACTIVE_HERO_VIDEO!==ACTIVE_HERO_PATH)throw new Error(`Unexpected active hero video: ${ACTIVE_HERO_VIDEO}`);
...
const requestedVideo=process.argv[2]??process.env.HERO_VIDEO_PATH;
if(requestedVideo){
  const requestedPath=fileReference(requestedVideo,'requested hero video');
  if(requestedPath!==ACTIVE_HERO_VIDEO)throw new Error(`Hero override must target the active hero video: ${requestedPath} != ${ACTIVE_HERO_VIDEO}`);
}
const HERO_VIDEO=ACTIVE_HERO_VIDEO;
...
console.log(JSON.stringify({file:repoPath(absoluteVideo),activeHeroFetch:ACTIVE_HERO_FETCH,activeHeroVideo:ACTIVE_HERO_VIDEO,bytes:bytes.length,minBytes:MIN_FILE_BYTES,maxBytes:MAX_FILE_BYTES,duration,maxDurationSeconds:MAX_DURATION_SECONDS,codec:stream.codec_name,width:stream.width,height:stream.height,maxEdgePixels:MAX_EDGE_PIXELS,maxPixelArea:MAX_PIXEL_AREA,videoStreams:1,audioStreams:0,boxes:{ftyp:true,moov:true,mdat:true,mdatPayloadBytes}},null,2));
