...
if (relativeVideo === ".." || relativeVideo.startsWith(`..${sep}`) || isAbsolute(relativeVideo)) {
...
let bytes;
try {
  const pathStat = lstatSync(absoluteVideo, { throwIfNoEntry: false });
  if (!pathStat) throw new Error(`Hero video not found: ${HERO_VIDEO}`);
  if (pathStat.isSymbolicLink()) throw new Error(`Hero video must not be a symbolic link: ${HERO_VIDEO}`);

  descriptor = openSync(absoluteVideo, constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0));
  ...
  if (pathStat.dev !== fileStat.dev || pathStat.ino !== fileStat.ino) {
    throw new Error(`Hero video changed while opening: ${HERO_VIDEO}`);
  }
  bytes = readFileSync(descriptor);
} catch (error) {
  if (error?.code === "ENOENT") throw new Error(`Hero video not found: ${HERO_VIDEO}`);
  if (error?.code === "ELOOP") throw new Error(`Hero video must not be a symbolic link: ${HERO_VIDEO}`);
  throw error;
} finally {
...
if (probe.error) {
  if (probe.error.code === "ETIMEDOUT") throw new Error("ffprobe timed out after 15000ms");
  throw probe.error;
}
if (probe.signal) throw new Error(`ffprobe was terminated (${probe.signal})`);
if (probe.status !== 0) throw new Error(`ffprobe failed (${probe.status}): ${probe.stderr.trim()}`);

const metadata = JSON.parse(probe.stdout);
const probedSize = Number(metadata.format?.size);
if (!Number.isFinite(probedSize) || probedSize !== bytes.length) {
  throw new Error(`${HERO_VIDEO} ffprobe size mismatch: ${metadata.format?.size ?? "unknown"}/${bytes.length}`);
}
