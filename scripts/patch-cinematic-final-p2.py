...
def replace_regex_once(source: str, pattern: str, replacement: str, label: str) -> str:
    ...
    if len(matches) != 1:
        raise RuntimeError(f"{label}: expected exactly one regex match, got {len(matches)}")
    return re.sub(pattern, replacement, source, count=1, flags=re.MULTILINE)


def replace_section(source: str, start_marker: str, end_marker: str | None, replacement: str, label: str) -> str:
    start = source.find(start_marker)
    if start < 0:
        raise RuntimeError(f"{label}: start anchor not found")
    end = len(source) if end_marker is None else source.find(end_marker, start + len(start_marker))
    if end < 0:
        raise RuntimeError(f"{label}: end anchor not found")
    return source[:start] + replacement + source[end:]


experience_pwa = '''const SERVICE_WORKER_PATH = "../sw.js?v=premium-cache-new-20260904-1";
...
'''
write("experience/experience-pwa.js", experience_pwa)

experience_html = read("experience/index.html")
experience_html = replace_regex_once(
    experience_html,
    r'  <script defer src="experience\.js\?v=[^"]+"></script>',
    '  <script defer src="experience-pwa.js?v=unbuilt"></script>\n\\g<0>',
    "experience HTML PWA runtime",
)
write("experience/index.html", experience_html)
...
service_worker = replace_regex_once(
    service_worker,
    r'(  "\./experience/index\.html",\n)(  "\./experience/experience\.js\?v=[^"]+",)',
    r'\1  "./experience/experience-pwa.js?v=unbuilt",\n\2',
...
security = read("scripts/verify-security-contracts.mjs")
...
for config_path in [
    ...
    config = replace_once(
        ...
        f"{config_path} experience URL",
    )
    write(config_path, config)

repeatability = read("scripts/run-lighthouse-repeatability.mjs")
new_load_runs = '''function classifyRepeatabilityRoute(finalUrl) {
  let parsed;
  try {
...
liminal_signals = read("scripts/build-liminalqa-signals.mjs")
old_lighthouse_block = '''if (!Array.isArray(lighthouse.profiles) || lighthouse.profiles.length !== 2) {
...
new_lighthouse_block = '''if (!Array.isArray(lighthouse.profiles) || lighthouse.profiles.length !== 2) {
...
const lighthouseVerdict = lighthouse.overallVerdict;'''
liminal_signals = replace_once(
    liminal_signals,
    old_lighthouse_block,
    new_lighthouse_block,
    "LiminalQA experience Lighthouse evidence",
)
write("scripts/build-liminalqa-signals.mjs", liminal_signals)

print("Applied final cinematic P2 fixes: root-scope PWA registration and experience-route repeatability evidence.")
