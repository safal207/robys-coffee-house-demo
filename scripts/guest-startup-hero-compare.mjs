...
const bodies = {
  baseline: parentBytes('qa.js'),
  'deduplicated-video': readFileSync('scripts/fixtures/guest-startup-hero-dedup.js')
};
if (hash(bodies.baseline) === hash(bodies['deduplicated-video'])) throw new Error('Candidate fixture is identical to baseline');
const temporary = mkdtempSync(path.join(process.cwd(), '.startup-hero-'));
const observations = [];
const metricNames = ['LayoutCount', 'LayoutDuration', 'RecalcStyleCount', 'RecalcStyleDuration', 'TaskDuration'];
const order = Array.from({ length: 3 }, () => ['baseline', 'deduplicated-video', 'deduplicated-video', 'baseline']).flat();
const report = {
  boundary: 'Controlled hero video initialization comparison; collection success is not product acceptance or pixel-presentation proof. ABBA repeated three times with a fresh Chromium process for every case. Both arms use the original release sampler and original appearance-check order, with identical callback start/end timestamps and CDP metrics. No animation wrapper, removed style reads, changed timing limits, dropped frames or CPU tracing. The only response-body treatment is qa.js; both arms intercept that request identically, so routing/cache effects are shared. Candidate bytes exist only in a diagnostic fixture and the fulfilled response, never in the served runtime file.',
  ...
  responses: Object.fromEntries(Object.entries(bodies).map(([variant, body]) => [variant, { path: '/qa.js', bytes: body.length, sha256: hash(body) }])),
  observations
};
...
try {
  ...
  for (const [index, variant] of order.entries()) {
    ...
    try {
      await certify({ port: 4280 + index, resultsDir, contract: 'STARTUP-HERO-COMPARISON-COLLECTION' }, async ({ browser, baseUrl }) => {
        observation.browser = browser.version();
        const context = await contextFor(browser);
        let page, session, beforeMetrics;
        try {
          await context.route(url => url.pathname === '/qa.js', async route => {
            const request = route.request();
            if (request.method() !== 'GET') throw new Error('Unexpected method for hero QA module');
          ...
          // Read media state only after the original done() and final CDP
          // snapshot. ResourceTiming counts completed retained entries, not all
          // network attempts: aborted/replaced requests can be absent.
          try {
            observation.hero = await page.evaluate(() => {
              const video = document.querySelector('.hero-video');
              const resources = performance.getEntriesByType('resource')
                .filter(entry => new URL(entry.name).pathname.endsWith('/src/robys-ambience-clean.mp4'))
                .map(entry => ({ path: new URL(entry.name).pathname, startTime: entry.startTime,
                  duration: entry.duration, responseEnd: entry.responseEnd, initiatorType: entry.initiatorType,
                  transferSize: entry.transferSize, encodedBodySize: entry.encodedBodySize }));
              return { observedAt: performance.now(), present: Boolean(video),
                source: video?.querySelector('source')?.getAttribute('src') ?? null,
                currentSrc: video?.currentSrc ?? null, readyState: video?.readyState ?? null,
                networkState: video?.networkState ?? null, paused: video?.paused ?? null,
                currentTime: video?.currentTime ?? null, videoWidth: video?.videoWidth ?? null,
                videoHeight: video?.videoHeight ?? null, playbackState: video?.dataset.playbackState ?? null,
                error: video?.error ? { code: video.error.code, message: video.error.message } : null,
                resourceEntryCount: resources.length, resources,
                resourceBoundary: 'Completed retained ResourceTiming entries; not a complete network request or reload count.' };
            });
            save(resultsDir, 'hero.json', observation.hero);
          } catch (error) { observation.heroDiagnosticError = error.message; }
          ...
          if (observation.routeFulfillments.length === 0) throw new Error('Hero QA module treatment was not requested');
        } catch (error) {
