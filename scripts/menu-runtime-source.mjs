...
export function compileMenuRuntime(source = readFileSync("src/menu-app.js", "utf8")) {
  ...
  const shellImport = 'import("./order-shell.js")';
  if (source.includes(shellImport)) {
    const shellRevision = createHash("sha256").update(readFileSync("order-shell.js")).digest("hex").slice(0,12);
    source = source.replace(shellImport, `import("./order-shell.js?v=${shellRevision}")`);
  }
